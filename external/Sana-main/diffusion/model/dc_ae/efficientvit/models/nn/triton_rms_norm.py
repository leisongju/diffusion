...
if triton is not None:
    TritonRMSNorm2dFunc = _make_triton_rms_norm()
else:
    ...
    class TritonRMSNorm2dFunc(torch.autograd.Function):
        ...
        def forward(ctx, x, weight, bias, eps):
            ...
            ctx.eps = eps
            rms = torch.sqrt(torch.square(x.float()).mean(dim=1, keepdim=True) + eps)
            return (x.float() / rms).to(x.dtype) * weight.view(1, -1, 1, 1) + bias.view(1, -1, 1, 1)
        ...
        def backward(ctx, dy):
            ...
            dw = (dy * x_normed).sum(dim=(0, 2, 3))
            db = dy.sum(dim=(0, 2, 3))
            c = x.shape[1]
            dx = dy * w.view(1, -1, 1, 1) * (1.0 / rms - x.float() * (1.0 / (rms ** 3)) / c).to(dy.dtype)
            return dx, dw, db, None
